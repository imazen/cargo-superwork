...
pub struct CiConfig {
    ...
    /// Pre-computed crate_name → git_url mapping for [patch.crates-io] generation in CI.
    /// Generated by `cargo superwork ci-patch-list`, stored here so CI environments
    /// that can't scan the full ecosystem still get correct patch entries.
    #[serde(default)]
    pub patch_repos: BTreeMap<String, String>,
}
